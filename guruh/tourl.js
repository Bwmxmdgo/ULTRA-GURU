...
async function handleUpload(from, Guru, conText, service) {
    ...
    // command as the caption on the media itself.
    const directImg      = mek.message?.imageMessage;
    const directVideo     = mek.message?.videoMessage;
    const directAudio     = mek.message?.audioMessage;
    const directSticker   = mek.message?.stickerMessage;
    const directDocument  = mek.message?.documentMessage;
    ...
    const quotedAudio    = quoted?.audioMessage || quoted?.message?.audioMessage || directAudio;
    const quotedSticker  = quoted?.stickerMessage || quoted?.message?.stickerMessage || directSticker;
    const quotedDocument = quoted?.documentMessage || quoted?.message?.documentMessage || directDocument;

    if (!quotedImg && !quotedVideo && !quotedAudio && !quotedSticker && !quotedDocument) {
        return reply(`⚠️ Please reply to (or send directly with the caption) an image, video, audio, sticker or document.`);
    }
