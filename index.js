...
async function startGuru() {
    try {
        ...
        // ── Track the bot's own outgoing messages ────────────────────────────
        ...
        // owner input.
        //
        // We track by TEXT CONTENT, registered synchronously before the send
        // even starts — not by message ID after the send resolves — because
        // the self-chat echo can arrive back through the event stream before
        // an ID-based await finishes, which let messages slip through.
        global._botSentTexts = global._botSentTexts || new Set();
        const _origSendMessage = GuruSocket.sendMessage.bind(GuruSocket);
        GuruSocket.sendMessage = async (jid, content, options) => {
            const tracked = content?.text || content?.caption;
            if (tracked) {
                global._botSentTexts.add(tracked);
                setTimeout(() => global._botSentTexts.delete(tracked), 15_000);
            }
